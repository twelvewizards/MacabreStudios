function adjustNavDisplay() {
    if ($(window).width() > 767) {
        ...
        // Always show the black rectangle regardless of the screen size
        $('#black-rectangle').show(); 
        // Hide the mobile navigation icon when the screen is larger
        $('#mobile-nav-icon').hide();
    } else {
        $('#top-nav').hide();
        // Always show the black rectangle regardless of the screen size
        $('#black-rectangle').show();
        // Show the mobile navigation icon when the screen is smaller
        $('#mobile-nav-icon, #top-logo').show();
    }
}
